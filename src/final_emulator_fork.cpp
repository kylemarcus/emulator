...
void BuildX(double * X, struct Data4 * proximity,
        vector<struct Data3> & uniq_coord, struct Data5 & I, int i,
        struct Data2 * q, struct Data1 *p, int sample);
...
int main(int argc, char * argv[]) {
    ...
    if (rank == 0) {
        ...
        DEBUG_VAR(M);
        DEBUG_VAR(S);
        DEBUG_VAR(MACRO_SCALED_RADIUS);
        DEBUG_VAR(T);
        DEBUG_VAR(MAX_COUNT);
        DEBUG_VAR(TOO_FEW);
        DEBUG_VAR(MERGE_COUNT);
        DEBUG_VAR(MERGE_SAFE);
        DEBUG_VAR(NEWTON_ITERATIONS);
    ...
    //ONLY FOR RANK = 0
    int master_group[Ngroups], master_rank[Ngroups], master_flag[Ngroups],
            master_indicator[Ngroups], master_sum = S, prev_sample[Ngroups];
    int Netz[Ngroups];
        ...
        while (master_sum > 0) {
            ...
            for (j = 1; j <= Ngroups; j++) {
                ...
                if (master_flag[i] == 1 && master_indicator[i] > -1) {
                    ...
                    master_flag[i] = 0;
                    MPI_Irecv(&master_indicator[i], 1, MPI_INT, master_rank[i],
                            Nprocs + master_rank[i], MPI_COMM_WORLD,
                            &master_req[i]);
    ...
    if (rank == root && rank != 0) {
        ...
        system(create_table);
        DEBUG("Created talbe on Netezza for group" << group);
    ...
    if (rank == root && rank != 0) {
        ...
        MPI_Request req1[Ntasks], req2[Ntasks], req3[Ntasks], req4[Ntasks],
                req5[Ntasks];
        MPI_Status stat1[Ntasks], stat2[Ntasks], stat3[Ntasks], stat4[Ntasks],
                stat5[Ntasks];
        ...
        double **X;
        int icount = 0, fifo;
        int sample_record[Ntasks], uniq_record[Ntasks], return_count[Ntasks],
                index;
        int flag[Ntasks];
        ...
        /* TODO: hard coded 2048 might be a issue?? */
        ...
        while (sample < S) {
            ...
            for (i = 0; i < p->spatial_count; i++) {
                ...
                for (j = 0; j < q->neighbour_count; j++)
                    fscanf(fpipe, "%d,%d,%d,%d", &(r->uniq_id),
                            &(r->spatial_neighbour[j]),
                            &(r->uniq_id_neighbour[j]),
                            &(r->macro_neighbour[j]));
            ...
            if (sample == start_sample) {
                ...
                for (iproc = 0; iproc < Ntasks; iproc++) {
                    ...
                    while ((phm_neighbours_count[icount] == 0)
                            || (q->neighbour_count < TOO_FEW)) {
                    ...
                    indicator = 2;
                    MPI_Isend(&indicator, 1, MPI_INT, rank_list[iproc], 0,
                            MPI_COMM_WORLD, &req1[iproc]);

                    Netezza[iproc] = new struct Data6;
                    Netezza[iproc][0].Sample = sample;
                    Netezza[iproc][0].Uniq_record = proximity[icount].uniq_id;
                    Netezza[iproc][0].Return_count = 4
                            * resample_neighbours_count
                            * phm_neighbours_count[icount];

                    sample_record[iproc] = sample;
                    uniq_record[iproc] = proximity[icount].uniq_id;
                    return_count[iproc] = resample_neighbours_count
                            * phm_neighbours_count[icount];

                    count = q->neighbour_count;
                    X[iproc] = new double[(Ndim + 3) * (count + 1)];
                    BuildX(&X[iproc][0], proximity, uniq_coord, I, icount, q, p,
                            sample);
                    ...
                    MPI_Isend(&pass_count[iproc][0], 3, MPI_INT,
                            rank_list[iproc], 3, MPI_COMM_WORLD, &req2[iproc]);

                    MPI_Isend(resample_neighbours, resample_neighbours_count,
                            MPI_INT, rank_list[iproc], 1, MPI_COMM_WORLD,
                            &req3[iproc]);

                    MPI_Isend(&phm_neighbours[icount][0],
                            phm_neighbours_count[icount], MPI_INT,
                            rank_list[iproc], 2, MPI_COMM_WORLD, &req4[iproc]);

                    MPI_Isend(&X[iproc][0], (q->neighbour_count) * (Ndim + 3),
                            MPI_DOUBLE, rank_list[iproc], 4, MPI_COMM_WORLD,
                            &req5[iproc]);

                    Netezza[iproc][0].mean_and_variance = new double[4
                            * return_count[iproc]];
                    MPI_Irecv(Netezza[iproc][0].mean_and_variance,
                            4 * return_count[iproc], MPI_DOUBLE,
                            rank_list[iproc], Nprocs + rank_list[iproc],
                            MPI_COMM_WORLD, &req2[iproc]);
            ...
            while (icount < p->spatial_count) {
                ...
                for (iproc = 0; iproc < Ntasks; iproc++) {
                    ...
                    if (flag[iproc] == 1) {
                        ...
                        X[iproc] = new double[(Ndim + 3) * (count + 1)];
                        BuildX(&X[iproc][0], proximity, uniq_coord, I, icount,
                                q, p, sample);
                        ...
                        MPI_Isend(&pass_count[iproc][0], 3, MPI_INT,
                                rank_list[iproc], 3, MPI_COMM_WORLD,
                                &req2[iproc]);

                        if (indicator == 2)
                            MPI_Isend(resample_neighbours,
                                    resample_neighbours_count, MPI_INT,
                                    rank_list[iproc], 1, MPI_COMM_WORLD,
                                    &req3[iproc]);

                        MPI_Isend(&phm_neighbours[icount][0],
                                phm_neighbours_count[icount], MPI_INT,
                                rank_list[iproc], 2, MPI_COMM_WORLD,
                                &req4[iproc]);

                        MPI_Isend(&X[iproc][0],
                                (q->neighbour_count) * (Ndim + 3), MPI_DOUBLE,
                                rank_list[iproc], 4, MPI_COMM_WORLD,
                                &req5[iproc]);
                        ...
                        if (Netezza_count == MAX_COUNT) {
                            ...
                            if (stat_count == 1) {
                                ...
                                sprintf(Loading_file, NZLOAD_SH" %d %d", rank,
                                        group);
                                sprintf(pipename, "Netpipe%d", rank);
                                system(Loading_file);
                                ...
                                if (fid < 0)
                                    PRINT("COULD NOT OPEN THE PIPE FOR WRITING");
                        ...
                        Netezza[iproc] = new struct Data6;
                        Netezza[iproc][0].Sample = sample;
                        Netezza[iproc][0].Uniq_record =
                                proximity[icount].uniq_id;
                        Netezza[iproc][0].Return_count = 4
                                * resample_neighbours_count
                                * phm_neighbours_count[icount];
        ...
        while (sum != 0) {
            ...
            for (iproc = 0; iproc < Ntasks; iproc++) {
                ...
                if (flag[iproc] == 1 && track[iproc] == 1) {
                    ...
                    MPI_Send(&indicator, 1, MPI_INT, rank_list[iproc], 0,
                            MPI_COMM_WORLD);
    ...
    if (rank != root && rank != 0) {
        ...
        while (indicator != 0) {
            ...
            MPI_Recv(&pass_count[0][0], 3, MPI_INT, root, 3, MPI_COMM_WORLD,
                    &stat[3]);
            ...
            if (indicator == 2) {
                ...
                MPI_Recv(resample_neighbours, pass_count[0][0], MPI_INT, root,
                        1, MPI_COMM_WORLD, &stat[1]);
            ...
            MPI_Recv(phm_neighbours_index, pass_count[0][1], MPI_INT, root, 2,
                    MPI_COMM_WORLD, &stat[2]);

            MPI_Recv(X, pass_count[0][2] * (Ndim + 3), MPI_DOUBLE, root, 4,
                    MPI_COMM_WORLD, &stat[4]);
            ...
            for (i = 0; i < NEWTON_ITERATIONS; i++)
                sigma = newton_method(R, Rinv, G, beta, rhs, i, Ndim + 2, count,
                        Ysend, Y, log_corr_len_guess, log_max_corr_len,
                        log_min_corr_len, rank);
            ...
            // loop over resamples -- i.e.calculate mean and variance using phm_neighbours for every resample neighbour
            for (i = 0; i < pass_count[0][0]; i++) {
                ...
                Mean_and_Variance(mean_and_variance_vector, G, rhs, R, Rinv,
                        beta, Xsend, Ysend, pass_count[0][1], count, Ndim + 2,
                        sigma, log_corr_len_guess, ymax);
            ...
            MPI_Isend(mean_var, k, MPI_DOUBLE, root, Nprocs + rank,
                    MPI_COMM_WORLD, &req_wait);

            mean_and_variance_vector.erase(mean_and_variance_vector.begin(),
                    mean_and_variance_vector.end());

            //      MPI_Recv(&indicator,1,MPI_INT,root,0,MPI_COMM_WORLD,&stat[0]);

            MPI_Irecv(&indicator, 1, MPI_INT, root, 0, MPI_COMM_WORLD,
                    &req_ind);
            //      if ( iproc > 0 )
...
void BuildX(double * X, struct Data4 * proximity,
    ...
    //This loops over the neighbours of the centre to create data structure for building R
    for (j = 0; j < q->neighbour_count; j++) { 

        if (proximity[i].uniq_id == proximity[i].uniq_id_neighbour[j]
                && sample == proximity[i].macro_neighbour[j])
            continue;
        ...
        for (k = 0; k < Ndim; k++)
            X[l * (Ndim + 3) + k + 2] =
                    I.data[proximity[i].macro_neighbour[j]][k];

        X[l * (Ndim + 3) + Ndim + 2] =
                p->spatial[proximity[i].spatial_neighbour[j]].H;
...
void Send_To_Netezza(list<struct Data6> & Pipeout, int rank, int group, int fid) {
    ...
    while (k != Pipeout.end()) {
        ...
        for (i = 0; i < k->Return_count; i = i + 4) {
            ...
            if (k->mean_and_variance[i] != -1) {
                ...
                sprintf(output, "%d %d %d %d %lf %lf\n", k->Sample + 1,
                        k->Uniq_record + 1, int(k->mean_and_variance[i]),
                        int(k->mean_and_variance[i + 1]),
                        k->mean_and_variance[i + 2],
                        k->mean_and_variance[i + 3]);
    ...
    time(&stop);
    DEBUG("TIME TAKEN " << difftime(stop, start));
