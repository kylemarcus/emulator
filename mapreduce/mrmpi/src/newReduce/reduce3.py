...
class ReducePhase1:
  ...
  def __init__(self):
    ...
    for i in range(self.Ndim):
      self.Resamples_dict[i+1] = []

    resamples_weight_dict = {}
    for line in fp:
      match = re.findall('([\d.\-e]+)', line )
      index = int(match[0])
      my_list = match[1:self.Ndim+1]
      for num,value in enumerate(my_list):
        self.Resamples_dict[num+1].append(float(value))
      resamples_weight_dict[int(match[0])] = float(match[self.Ndim+1])

    #Scaling resamples data
    for key in self.Resamples_dict:
      my_list = self.Resamples_dict[key]
      min_list = min(my_list)
      max_list = max(my_list)
      self.Resamples_dict[key] = scale(my_list,max_list,min_list)


    Uncertain_List_Obj = Build_Sample_Object()
    self.Samples_dict = Uncertain_List_Obj.get_hashed_data()

    sample_neigh_count_dict = {}
    fp = open("/panasas/scratch/shivaswa/emulator/my_hadoop/resample_count.txt","r")
  ...
  def main(self, inputfp):
    ...
    for line in inputfp:
      ...
      resample_list = []
      resample_list = [self.Resamples_dict[i+1][resample-1] for i in range(self.Ndim)]
      dist = ComputeDist(self.Samples_dict[sample],resample_list)

      if key != new_key:
        if count != 0:
          match = re.search(r'(\d+)_(\d+)',key)
          output += [ str(match.group(2)) + ' ' + str(match.group(1)) + ' ' + str(obj.get_weighted_mean()) ]
