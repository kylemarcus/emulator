...
DataStruct() {}
DataStruct(string arg1, int arg2, double arg3) : uniq_id(arg1), phm_id(arg2), mean(arg3) {}
~DataStruct() {}
void display() { cout << "uniq_id = " << uniq_id << "   phm_id = " << phm_id << "    mean = " << mean << endl; }
};

struct DataList {
int resample;
list <DataStruct> output;
...
void Mean_and_Variance(matrixop & G, matrixop & Rinveps, matrixop & R, matrixop & Rinv, matrixop & beta, struct Data5 & xsx, struct Data5 & ysy, int Nx, int Ny, int Ndim, double sigma, double * log_corr_len_guess, double ymax, int sample, string uniq_coord, int resample, vector<int> phm,DataList &  my_list);
...
//void Mean_and_Variance(matrixop & G, matrixop & Rinveps, matrixop & R, matrixop & Rinv, matrixop & beta, struct Data5 & xsx, struct Data5 & ysy, int Nx, int Ny, int Ndim, double sigma, double * log_corr_len_guess, double ymax, int sample, string uniq_coord, int resample, vector<int> phm)
//void Mean_and_Variance(matrixop & G, matrixop & Rinveps, matrixop & R, matrixop & Rinv, matrixop & beta, Data5 & xsx, Data5 & ysy, int Nx, int Ny, int Ndim, double sigma, double * log_corr_len_guess, double ymax)
void Mean_and_Variance(matrixop & G, matrixop & Rinveps, matrixop & R, matrixop & Rinv, matrixop & beta, struct Data5 & xsx, struct Data5 & ysy, int Nx, int Ny, int Ndim, double sigma, double * log_corr_len_guess, double ymax, int sample, string uniq_coord, int resample, vector<int> phm, DataList & my_list)
{
int i,j,k;
...
//cout << " resample = " << my_list.resample << "   size = " << (my_list.output).size() << endl;
...
//if ( sum <= 1.5*ymax )  {
   ...
   while ( phm_iter != (my_list.output).end() ) {
	if ( phm[i] > phm_iter->phm_id )  phm_iter++;
	else break;
   }

   double mean = ( sum > 0 ) ? sum:0;
   mean = (mean<=ymax*1.5) ? mean : -1;
   DataStruct temp_output = DataStruct(uniq_coord, phm[i], mean);
   (my_list.output).insert(phm_iter, temp_output);

//cout << sample << " " << uniq_coord << " " <<  resample << " " << phm[i] << " " << (( sum > 0 ) ? sum:0) << endl;
}
