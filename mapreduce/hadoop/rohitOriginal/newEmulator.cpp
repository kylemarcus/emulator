#include <iostream>
#include <vector>
#include "/user/shivaswa/my_hadoop/matrixop.h"
#include "/user/shivaswa/my_hadoop/new_build_emulator.h"
#include <list>
#include <map>
#include <sstream>
#include <stdio.h>
...
//	DEBUG()
	...
	sprintf(task_filename,"/user/shivaswa/my_hadoop/Aug_1/BLAH_AUG_1_%d.txt",sample);
	...
	phm_count_list[i] = 0;
	char filename[50];
	sprintf(filename,"/panasas/scratch/shivaswa/phm_count_files/phm_count_%d.txt",sample);
	fp = fopen(filename,"r");
	while ( !feof(fp) ) {
	 int index, count;
...
//	output_filename << "/panasas/scratch/shivaswa/output/file" << "_" << sample << "_" << rand()%1000000;
	output_filename << "/panasas/scratch/shivaswa/emulator_output/file" << "_" << sample << "_" << count;
	if ( !out_file.is_open() )
...
//	  cout << " uniq_count = " << count << endl;
	  Mean_and_Variance(G, rhs, R, Rinv, beta, Res_X[iresample], X, Mpts, Npts, Ndim+2, sigma, log_corr_len_guess, ymax, sample,uniq_coord,res_neigh[iresample], phm_neigh, *my_iter, phm, x_y_coord, phm_count_list, out_file);
