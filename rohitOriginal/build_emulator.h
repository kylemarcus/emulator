...
double newton_method(matrixop & R,matrixop & Rinv, matrixop & G, matrixop & beta, matrixop & rhs,int inewt, int Ndim, int Npts, struct Data5 & X,double * y, double * log_corr_len_guess, double * log_max_corr_len, double * log_min_corr_len, int rank);
void Mean_and_Variance(vector<double> & mean_and_variance, matrixop & G, matrixop & Rinveps, matrixop & R, matrixop & Rinv, matrixop & beta, struct Data5 & xsx, struct Data5 & ysy, int Nx, int Ny, int Ndim, double sigma, double * log_corr_len_guess, double ymax);
...
///////////////////////////////////////////////////////////////////    MODIFIED ON JULY 19   ///////////////////////////////////////////
double Rcond,worstcaseleigmax,nugget;
Rcond = R.rcond();
/*
...
if ( sum <= 1.5*ymax )
mean_and_variance.push_back( (sum > 0) ? sum : 0 );
else
mean_and_variance.push_back(-1);


sum = sum1[i] + 1 - sum2[i];				///////////////////////////    ADDED ON JULY 19 3:15pm    //////////////////////////////
sum = (sum > 20000)?20000:sum;
mean_and_variance.push_back(sigma* ( sum > 0 ? sum:1)  );
