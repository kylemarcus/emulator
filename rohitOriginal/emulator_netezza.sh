...
#emulator_netezza.sh as the neame sugests is to be run on the Netezza server.
#Its SQL file with a shell wrapper that allows one to run the script from the front-end of Netezza. 
#so log on netezza.ccr.buffalo.edu and ensure that the files rohit.txt, meta_data.txt, montserrat_take2_vol_dir_bed_int.ph, macro_resamples.tmp
#and uncertain_input_list.txt are in the home folder which was "/home/shivaswa/final/" in my case. 
...
CREATE EXTERNAL TABLE meta_ex(sample int, Nx int, Ny int, xstart double precision, xend double precision, ystart double precision, yend double precision,logvol double precision,direction double precision,basal double precision,internal double precision) using (dataobject('/home/shivaswa/final/meta_data.txt') delimiter',');
...
CREATE EXTERNAL TABLE resamples_ex(resample int,P1 double precision,P2 double precision,P3 double precision,P4 double precision, w double precision) using (dataobject('/home/shivaswa/final/macro_resamples.tmp') delimiter ' ');
...
CREATE EXTERNAL TABLE phm_ex(id int,X double precision, Y double precision) using (dataobject'/home/shivaswa/final/montserrat_take2_vol_dir_bed_int.phm' delimiter ' ');
