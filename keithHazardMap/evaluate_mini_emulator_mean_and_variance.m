function evaluate_mini_emulator_mean_and_variance(samplenumber,macrosimplex_resample_filename)
    iftoc=1;
    if(iftoc)
        tic;
    end
...
%         fprintf(fid,'%%{{iyneigh: index of y neighbor: [1] integer},{ihop: #of (y0,ymacro) hops from center of micro-emulator: [3] integers},{Rinveps: R^-1*eps (eps = sy minus unadjusted micro-emulator mean): [1] double}}\n');
    ...
    if(checksamplenumber~=samplenumber)
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE1;
        disp(sprintf('ERROR: file "%s" claims to be mini-emulator #%g instead!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE1.mat\n',miniemulatorfilename,checksamplenumber));
    ...
    for iy0=1:Ny0N
        ...
        if(isempty(checkiy0)||(iy0~=checkiy0))
            save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE2;
            disp(sprintf('ERROR: iy0=%g ~= checkiy0=%g!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE2.mat\n',iy0,checkiy0));
            bob; %cause the code to crash
        end
        ...
        pwem(iy0).sigma=sscanf(fgets(fid),'%g',1); %don't need to retain this for adjusted mean
        pwem(iy0).beta =sscanf(fgets(fid),betaformat,Ndimin+1);
        
        Nneigh0=sscanf(fgets(fid),'%g',1); %don't need to retain this for adjusted mean or variance
        yada=fscanf(fid,'%g %g\n',[2 Nneigh0])'; %[iy0neigh i0hop], don't need to retain these for adjusted mean or variance

        %fgets(fid) %read in the \n that fscanf left on the end of the previous line    
            
        Nneigh=sscanf(fgets(fid),'%g',1);  %don't need to retain this for adjusted mean or variance
        yada=fscanf(fid,'%g %g %g %g %g\n',[5 Nneigh])'; %[iyneigh ihop Rinveps], don't need to retain ihop for adjusted mean or variance
...
%         fprintf(fid,'%%     [Nxspatial] lines containing {{(east,north): [2] doubles},{[Nmacroseachx] sets of {(log10(volume [m^3]),Direction [deg CC from east],BedFrictAng [deg],IntFrictAng [deg]): macro input coordinates: [Ndiminmacro] doubles}}}\n');
    ...
    UniqueMiniEmulatorKey=sprintf('%s.-2.%06d',UniqueMacroSimplexKey,samplenumber); %the -2 in the .-2. indicates it's an evaluation of the adjusted first and second moments (mean and variance)
    
    Ndiminmacro=sscanf(fgets(fid),'%g',1);
    if(Ndimin-(Ndiminspatial+Ndiminmacro))
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE3;
        disp('ERROR: Ndimin~=Ndiminspatial+Ndiminmacro!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE3.mat\n');
        bob; %cause the code to crash
    end
    
    yada=sscanf(fgets(fid),sprintf('(%%g)%s',repmat(' %g',1,Ndiminmacro+1)),Ndiminmacro+2);
    if(yada(1)~=iMacroSimplex)
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE4;
        disp(sprintf('ERROR: file "%s" claims it''s for simplex %g!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE4.mat\n',macrosimplex_resample_filename,yada(1)));
        bob; %cause the code to crash
    end
    ...
    if(isempty(iMacroSimplexNode))
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE5;
        disp(sprintf('ERROR: file "%s" claims its only macro simplex nodes are %s but this mini-emulator (#%g) is not one of them!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE5.mat\n',...
    ...
    if(Ndiminspatial~=checkNdiminspatial)
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE6;
        disp(sprintf('ERROR: file Ndiminspatial=%g ~= checkNdiminspatial=%g!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE6.mat\n',...
    ...
    if(~(Nmacroseachx>0))
        save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE7;
        disp(sprintf('ERROR: Nmacroseachx=%g is not greater than zero!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE7.mat\n',Nmacroseachx));
        bob; %cause the code to crash
    end
    ...
    switch(fileformatoption)
        case 1,
            ...
            xsx=[repmat(xspatial,Nmacroseachx,1) reshape(repmat(Xmacro',Nxspatial,1),Ndiminmacro,Nxspatial*Nmacroseachx)' zeros(Nx,Nmoments)];
        ...
        case 2,            
            ...
            if(Nmacroseachx==1)
                ...
                xsx=[repmat(xspatial,Nmacroseachx,1) ...
                     reshape(yada(:,ii),Nx,Ndiminmacro) zeros(Nx,Nmoments)];                
                clear yada xspatial Nxspatial Nmacroseachx ii;
            end
        otherwise,
            save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE8;
            disp(sprintf('ERROR: unknown 2nd input file format option!!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE8.\n'));
            bob; %cause the code to crash
    end   
    ...
    for imicrosimptoeval=1:Nmicrosimptoeval
        ...
        for inode=1:Nnodes
            ipwem=Iy0nodes(inode);
             xsxS=add_micro_emulator_adjusted_mean_and_variance(pwem(ipwem),...
...
%                with tsearchn
    ...
    switch(fileformatoption)
        ...
        case 2,
            ...
            fprintf(fid,yada,xsx');            
        otherwise,
            save DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE9;
            disp(sprintf('ERROR: I don''t know how to handle file format %d!!! Workspace saved to DEBUG_EVALUATE_MINI_EMULATOR_MEAN_AND_VARIANCE9.mat\n',fileformatoption));
            bob; %cause the code to crash
    end
...
function xsx=add_micro_emulator_adjusted_mean_and_variance(pwem,ysy,xsx,p)
    %if(~pwem.sigma)
    %    return;
    %end
    ...
    negZ=zeros(Ny^2,Ndimin);
    for idimin=1:Ndimin
        y=repmat(ysy(:,idimin),[1,Ny]);
        negZ(:,idimin)=reshape((y-y').^2,[Ny^2 1]);
    end

    R=reshape(exp(negZ*(-pwem.thetaTRAN)),Ny,Ny);
    ...
    rTRAN=reshape(exp(negZ*(-pwem.thetaTRAN)),Nx,Ny);
    %save DEBUGMEWTF_var;
    xsx(:,Idimout(1))=xsx(:,Idimout(1))+p.*(...
        max([ONES xsx(:,Idimin)-repmat(ysy(1,Idimin),Nx,1)]*pwem.beta,0)+rTRAN*pwem.Rinveps);
    xsx(:,Idimout(2))=xsx(:,Idimout(2))+p.*(...
        pwem.sigma*(1-sum((rTRAN/R).*rTRAN,2)));
return;